...
MMU::MMU(std::shared_ptr<Cartridge> inCartridge, CPU& inCPU, Video& inVideo, Input& inInput, Serial_gba& inSerial_gba, Timer& inTimer, Options& inOptions) :
    cartridge(inCartridge),
    cpu(inCPU),
    video(inVideo),
    input(inInput),
    serial_gba(inSerial_gba),
    timer(inTimer),
    options(inOptions)
{
...
u8 MMU::read_io(const Address& address) const {
    switch (address.value()) {
        ...
        case 0xFF01:
            return serial_gba.read();

        case 0xFF02:
            log_unimplemented("Attempted to read serial_gba transfer control");
            return 0xFF;
...
void MMU::write_io(const Address& address, const u8 byte) {
    switch (address.value()) {
        ...
        case 0xFF01:
            /* Serial_gba data transfer (SB) */
            serial_gba.write(byte);
            return;

        case 0xFF02:
            /* Serial_gba data transfer (SB) */
            serial_gba.write_control(byte);
            return;
